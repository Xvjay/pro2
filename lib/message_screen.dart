...
class _MessageScreenState extends State<MessageScreen> {
                ...
                child: ListTile(
                  ...
                  onTap: () {
                    Navigator.push(context, MaterialPageRoute(
                      builder: (context) => ChatWindow(boardId: board.id, boardName: board.title),
                    ));
                  },
                ),
              );
  ...
  Stream<List<MessageBoard>> getMessageBoardsStream() {
    return FirebaseFirestore.instance
        ...
        .snapshots()
        .map((snapshot) =>
            snapshot.docs.map((doc) => MessageBoard.fromFirestore(doc)).toList());
  }
...
class MessageBoard {
  ...
  MessageBoard({required this.id, required this.title, this.imageUrl = 'default_image.png'});
